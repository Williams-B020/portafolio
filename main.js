  ...
  function finish() {
    ...
    panels.addEventListener(
      ...
      () => {
        ...
        setTimeout(() => loaderWrap.remove(), 600);
      },
      { once: true }
    );
  }

  // Si no hay imágenes, termina de inmediato
  if (totalImages === 0) {
    finish();
  } else {
    images.forEach((img) => {
      if (img.complete) {
        ...
        img.addEventListener("error", imageLoaded); // por si alguna falla
      }
    });
  }
});
